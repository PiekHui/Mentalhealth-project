...
class PetModel {
  ...
  Future<Map<String, dynamic>> loadPetData() async {
    ...
    if (doc.exists) {
      Map<String, dynamic> defaultData = Map.from(
        _petData,
      ); // Start with defaults
      ...
      if (serverData != null) {
        ...
        _petData = Map.from(defaultData);
      }

      // Convert relevant timestamps
      _petData['lastInteractionTimes'] = _convertTimestamps(
        _petData['lastInteractionTimes'] ?? {},
      );
      ...
      _petData['lastTaskStatusResetDate'] = _convertToDateTime(
        _petData['lastTaskStatusResetDate'],
      );
      ...
      _petData['lastHappinessUpdateTime'] = _convertToDateTime(
        _petData['lastHappinessUpdateTime'],
      );
      ...
      _petData['diaryEntriesToday'] =
          _petData['diaryEntriesToday'] as int? ?? 0;
      ...
      _petData['happiness'] =
          _petData['happiness'] as int? ?? 80; // Ensure happiness exists

      // Ensure task status is the correct type
      _petData['dailyTaskStatus'] = Map<String, bool>.from(
        _petData['dailyTaskStatus'] ?? {},
      );
      ...
      if (lastUpdate != null) {
        ...
        final int hoursPassed =
            timeDifference.inHours; // Get whole hours passed

        if (hoursPassed > 0) {
          print(
            "[loadPetData] $hoursPassed hour(s) passed since last happiness update ($lastUpdate). Applying decay.",
          );
          ...
          final int newHappiness = math.max(
            0,
            currentHappiness - totalDecrease,
          ); // Don't go below 0

          print(
            "[loadPetData] Happiness decay: $currentHappiness -> $newHappiness (-$totalDecrease)",
          );
          ...
          timeBasedUpdates['lastHappinessUpdateTime'] =
              FieldValue.serverTimestamp();
          _petData['lastHappinessUpdateTime'] =
              now; // Update local cache immediately
        } else {
          print(
            "[loadPetData] Less than an hour passed since last happiness update ($lastUpdate). No decay applied.",
          );
        }
      } else {
        // If timestamp is missing for some reason, set it now
        print(
          "[loadPetData] lastHappinessUpdateTime was null. Setting it to now.",
        );
        timeBasedUpdates['lastHappinessUpdateTime'] =
            FieldValue.serverTimestamp();
        _petData['lastHappinessUpdateTime'] =
      ...
      if (isNewDayForTasks) {
        print(
          "[loadPetData] New day detected for tasks. Resetting task status.",
        );
        dailyResets['dailyTaskStatus'] = <String, bool>{};
        dailyResets['lastTaskStatusResetDate'] = FieldValue.serverTimestamp();
        _petData['lastTaskStatusResetDate'] =
            now; // Update local date immediately
      }

      // --- ADDED: Reset new task counters ---
      if (isNewDayForTasks) {
        // Only reset these if it's a new task day
      ...
      final Map<String, dynamic> allUpdates = {
        ...timeBasedUpdates,
        ...dailyResets,
      };
      ...
      if (allUpdates.isNotEmpty) {
        ...
        await updatePetData(
          allUpdates,
        ); // Persist combined updates to Firestore
      }
      // --- End Apply Updates ---
  ...
  Future<void> feedPet() async {
    try {
      ...
      final happiness = math.min(
        100,
        (_petData['happiness'] ?? 0) + 5,
      ); // Small boost
  ...
  Future<void> petPet() async {
    try {
      ...
      final happiness = math.min(
        100,
        (_petData['happiness'] ?? 0) + 8,
      ); // Slightly larger boost
  ...
  Future<void> playWithPet() async {
    try {
      ...
      final happiness = math.min(
        100,
        (_petData['happiness'] ?? 0) + 12,
      ); // Good boost
      final experience =
          (_petData['experience'] ?? 0) + 10; // Keep experience gain
  ...
  Future<void> saveLearnedTrick(String trick) async {
    final updates = {
      ...
      'experience':
          ...
          15, // Learning tricks gives experience
    };
    await updatePetData(updates);
  }
  ...
  Future<void> updateExperienceAndLevel(int newExperience, int newLevel) async {
    final updates = {'experience': newExperience, 'level': newLevel};
    await updatePetData(updates);
  }
      ...
      return achievements.map((achievement) {
        if (achievement['date'] is Timestamp) {
          achievement['date'] =
              achievement['date']
                  .toDate()
                  .toString(); // Convert date for display
        }
        if (achievement['color'] is int) {
          achievement['color'] = Color(
            achievement['color'],
          ); // Recreate Color object
        }
  ...
  Future<void> checkForNewAchievements() async {
    ...
    final currentAchievementTitles = Set<String>.from(
      achievements.map((a) => a['title']),
    );

    // Level achievements
    if (currentPetData['level'] >= 5 &&
        !currentAchievementTitles.contains('Level 5')) {
      await addAchievement('Level 5', 'star', Colors.amber, 10);
    }
    if (currentPetData['level'] >= 10 &&
        !currentAchievementTitles.contains('Level 10')) {
      await addAchievement('Level 10', 'star', Colors.amber, 20);
    }

    // Trick achievements
    if ((currentPetData['learnedTricks']?.length ?? 0) >= 3 &&
        !currentAchievementTitles.contains('Trick Master')) {
    ...
    if (currentPetData['happiness'] >= 90 &&
        !currentAchievementTitles.contains('Happy Pet')) {
    ...
    try {
      ...
      await docRef.set({
        ...
        'lastMessageTimestamp':
            messageData['timestamp'], // Keep track of last message time for sorting dates
  ...
  Future<List<Map<String, dynamic>>> loadChatHistoryForDate(
    String dateString,
  ) async {
